...
async fn main() -> anyhow::Result<()> {
    ...
    let config = {
        let config_file =
            solana_cli_config::CONFIG_FILE.as_ref().ok_or_else(|| {
                anyhow!(
                    "unable to determine a config file path on this OS or user"
                )
            })?;
    ...
    let signer =
        signer_from_path(&matches, &keypath, "keypair", &mut wallet_manager)
